...
-- 4) 학과별로 26살 이상인 학생 전체 컬럼 조회(이름, 핸드폰번호, 나이, 학과로 별칭붙여서)
...
--WHERE STU_NAME IN('김%', '이%')
GROUP BY STU_DEPT;

-- 6) 컴퓨터 공학과의 학생중 25살이상인 학생수 조회
SELECT COUNT(*) AS "25살이상인 컴공과학생"
FROM TBL_STU_G
...
-- 내부쿼리2 : ROWNUM 정렬된 급여 데이터 RN 순번 부여
