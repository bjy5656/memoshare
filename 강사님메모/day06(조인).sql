...
-- 1) 행의 개수를 먼저 파악하여 선행 테이블, 후행테이블 결정
SELECT COUNT(*)
FROM TBL_BOOKS t;
...
-- 3) 두개의 테이블에서 일치하는 컬럼을 확인후 ON절에 추가
...
-- 정확히 같은 값을 가진 데이터로만 조회 1 = 1, 2 = 2, .. 1 -> 3
SELECT TB.BOOK_TITLE, TM.MEM_NAME, TB.BOOK_ID, TM.MEM_ID
...
-- 책대여일-반납일 사이에 특정 날짜가 포함되었는지 조회(2023-12-25)
...
WHERE REN_RENTALDATE BETWEEN TO_DATE('2023-12-20', 'YYYY-MM-DD') AND TO_DATE('2023-12-20', 'YYYY-MM-DD');  
