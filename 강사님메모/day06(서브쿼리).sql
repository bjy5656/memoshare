...
--FROM EMPLOYEES; -- 조회하려는 행의 개수가 달라서 오류발생
...
-- 부서이름까지 같이 조회하고싶다? => 서브쿼리를 2번쓰거나 JOIN을 이용해야한다
...
FROM EMPLOYEES
GROUP BY DEPARTMENT_ID; -- 결과 12개 행

-- 메인쿼리의 결과 행수보다 서브쿼리의 행의 수가 많으면 오류 발생!!
--SELECT AVG(SALARY), (SELECT * FROM EMPLOYEES)
--FROM EMPLOYEES;

-- 서브쿼리 사용 시에는 * 전체를 조회하는 경우 앞에 테이블별칭을 붙여서 테이블별칭.* 형태로 조회해야한다
...
-- 1. SELECT 절 : 계산된 값 또는 추가 정보를 열로 표시
-- 각 직원의 사원 번호, 이름, 급여와 해당부서의 평균 급여 조회
...
-- 2. FROM절 서브쿼리 : 데이터 소스 역할, 주로 집계 데이터를 연결할 때 사용
